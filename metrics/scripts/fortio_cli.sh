...
tcp() {
    ...
    do
        ...
        REPORT="${FILE_PATH}/$(echo ${Label}|sed s"/ /_/g")_${NAMESPACE}_${c}c_${DATE}.json"
        echo "Running ${Label} for ${DURATION}s with $c connections in K8s ns $NAMESPACE"
        kubectl -n $NAMESPACE --context ${K8S_CONTEXT} exec -i deploy/fortio-client -c fortio -- fortio load -qps ${QPS} -c $c -r .0001 -t ${DURATION}s ${HEADERS} -payload "${PAYLOADBYTES}" -a -labels "${Label}" ${JSON} tcp://fortio-server-defaults:8078 > "${REPORT}"
        sleep $RECOVERY_TIME
        report $REPORT $DATE ${NAMESPACE}
    done
    echo
...
get_cpu_throttled(){
    ...
    if [[ ${NS} =~ .*baseline.* ]]; then
        ...
        CONTAINER="consul-dataplane|istio-proxy"
    fi
...
if [[ ! -z $FILE_PATH && -z $JSON ]]; then
    echo "FILE_PATH '-f' was set for reporting without enabling JSON output"
    echo "Enabling '-j' to Redirect JSON Output to STDOUT for reporting"
    JSON="-json -"
fi
if [[ -z $FILE_PATH ]]; then
    FILE_PATH="/tmp"
...
if [[ -z $TEST ]]; then
    echo "Running Default Test for http"
    TEST="http"
fi
$TEST