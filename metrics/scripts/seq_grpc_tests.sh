...
baseline(){
    ...
    do
        if [[ ${type} =~ consul_http ]]; then
            ...
            ${SCRIPT_DIR}/fortio_cli.sh  -j -t baseline_http -n fortio-baseline -k${K8S_CONTEXT} -d${DURATION} -w${RECOVERY_TIME} -c${c} -h ${HEADER} -p ${PAYLOAD} -f ${FILE_PATH}
            pid=$!
            wait $pid
            echo "Test completed (PID: $pid)"
        else
            echo echo "Running Test Type 'baseline_grpc' in namespace fortio-baseline with $c connections"
            ${SCRIPT_DIR}/fortio_cli.sh  -j -t baseline_grpc -n fortio-baseline -k${K8S_CONTEXT} -d${DURATION} -w${RECOVERY_TIME} -c${c} -h ${HEADER} -p ${PAYLOAD} -f ${FILE_PATH}
            pid=$!
            wait $pid
        fi
    done
